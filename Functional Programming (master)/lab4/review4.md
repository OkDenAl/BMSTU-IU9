...
# Вывод
Уже имея опыт написания интерпретаторов, 
я смог сосредоточиться на особенностях функционального подхода в Haskell.
Главное открытие - насколько элегантно управление состоянием решается
через чистые функции и неизменяемые структуры данных.

В отличие от императивных языков, где состояние обычно мутабельное,
здесь пришлось выстраивать четкую цепочку трансформаций состояния
через каждый шаг интерпретации. Это заставляет думать иначе,
но результат получается более надежным и тестируемым.

Особенно интересно было реализовывать стековую архитектуру Forth она идеально
ложится на функциональную парадигму. 
Управляющие конструкции (циклы, условия) потребовали аккуратного проектирования,
но механизм пропуска кода и стека циклов оказался очень логичным.

Из технических сложностей - обработка строк
и корректное восстановление состояния при вызове
пользовательских слов потребовали нескольких итераций. Также иногда приходилось
подумать над примерами для тестов.

В итоге получился полнофункциональный интерпретатор,
поддерживающий все основные конструкции Forth.
Работа подтвердила, что функциональный подход отлично подходит
для реализации языков программирования, особенно с простым синтаксисом.

Однозначно могу сказать, что это была интересная лаба.