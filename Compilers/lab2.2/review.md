...
[Func(header=FuncHeaderFull(type=ArrayType(type=<BasicType.Integer: 'int'>),
      ...
      body=[AssignStatement(variable=BasicVar(type=<BasicType.Integer: 'int'>,
            ...
            AssignStatement(variable=BasicVar(type=ArrayType(type=<BasicType.Integer: 'int'>),
                            ...
                            expr=NewAllocStatement(type=ArrayType(type=<BasicType.Integer: 'int'>),
                                                   alloc_size='10')),
            ForStatement(header=ForHeaderShort(for_val=CycleVar(type=ArrayType(type=<BasicType.Integer: 'int'>),
                                                                var_name='#i'),
...
такой записью я познакомился в рамках предыдущих лабораторных работ по курсу Конструирование Компиляторов).
Так что я её заменил на библиотеку, поддерживающую такое написание. Вот так :)